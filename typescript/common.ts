...
import { readableStreamToString, TexthiveError, parseIfJson } from "./error";
            ...
            if (e instanceof AxiosError && e.isAxiosError) {
                ...
                try {
                    const responseBody =
                        ...
                        : e.response?.data
                    throw new TexthiveError(e, parseIfJson(responseBody), e.response?.headers)
                } catch (innerError) {
                    if (innerError instanceof ReferenceError) {
                        ...
                        throw new TexthiveError(e, e.response?.data, e.response?.headers)
                    }
                    if (innerError instanceof TexthiveError) {
                        // Got "TexthiveError" from the above try block
                        throw innerError;
                    }
